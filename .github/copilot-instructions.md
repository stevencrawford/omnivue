...
- Custom hooks: `useSSE.ts` (SSE subscription with auto-reconnect), `useApi.ts` (typed API fetch wrappers), `useTheme.ts` (theme state/persistence), `useNav.ts` (session nav context).
...
- **Persistent search**: FTS5 in `sess.db` indexes all session content incrementally with content-hash dedup.
