...
- 2026-08-04 — ATH-09 (split god components) in progress on `refactor/ath09-god-components`: SettingsModal split 928→85-line shell composing per-tab modules (AgentSettingsTab, SessionsSettingsTab, AppearanceSettingsTab, Privacy/Developer tabs, AboutSettingsTab, each <400 lines); extracted shared `FilterChip` (deduped from SettingsModal + SessionPanel), `useHideCosts` hook, `utils/uuid.ts` `makeId`. Remaining god components not yet split: SessionPanel (838), TagPanel (677), DiffView (677), SessionSummary (648), OverviewScreen (434); `ProjectPanel` no longer exists. Frontend gates green; row stays `open` until named components are all <400 lines.
