...
// ScanJSONL opens a JSONL file, iterates over each line, and calls fn for
// each unmarshaled line. The file is closed after iteration.
...
// structured task list. Intended to detect messages that contain TODO lists,
// step-by-step plans, or implementation checklists generated by an LLM.
...
const (
	// DefaultJSONLBufferSize is the buffer size for bufio.Scanner when reading JSONL lines.
	// File content embedded in JSONL lines can exceed the default 64KB limit; the largest
	// lines (Copilot events with embedded file contents) require up to 10MB.
	DefaultJSONLBufferSize = 10 * 1024 * 1024
