...
export function EditTagModal({ isOpen, tag, onClose, onSave }: EditTagModalProps) {
  ...
  useEffect(() => {
    if (isOpen && tag) {
      setName(tag.name);
      setColor(hasTagColor(tag.color) ? (tag.color ?? TAG_NO_COLOR) : TAG_NO_COLOR);
      setSaving(false);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await onSave(trimmed, color || undefined);
    setSaving(false);
    onClose();
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Tag" size="md">
