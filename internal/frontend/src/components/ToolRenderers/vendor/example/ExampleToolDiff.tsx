  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2 px-2.5 py-1.5 text-[11px] font-mono min-w-0">
        ...
        <span className="text-gh-text truncate min-w-0">{tool.name}</span>
      </div>
    );
  }
  ...
  return (
    <div className="border border-gh-border rounded-lg overflow-hidden mb-3 bg-gh-bg-secondary/50">
      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-accent-border bg-gh-bg-secondary/50 text-[11px] font-mono text-gh-text-secondary">
        <span className="font-medium text-gh-text">{kind}</span>
      </div>
      <div className="px-3 py-2 space-y-2">
        ...
        {tool.output && (
          <div>
            ...
            <pre className="mt-0.5 p-2 bg-gh-bg rounded text-[11px] font-mono overflow-x-auto text-gh-text whitespace-pre-wrap">
              {tool.output}
            </pre>
          </div>
        )}
